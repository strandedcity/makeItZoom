...
d(b[15])+")"},w=function(a,d){if(a instanceof THREE.CSS2DObject){var c;a instanceof THREE.CSS3DSprite?(b.copy(d.matrixWorldInverse),b.transpose(),b.copyPosition(a.matrixWorld),b.scale(a.scale),b.elements[3]=0,b.elements[7]=0,b.elements[11]=0,b.elements[15]=1,c=t(b)):c=t(a.matrixWorld);var e=a.element,f=r[a.id];if(void 0===f||f!==c)e.style.WebkitTransform=c,e.style.MozTransform=c,e.style.oTransform=c,e.style.transform=c,r[a.id]=c;e.parentNode!==h&&h.appendChild(e)}c=0;for(e=a.children.length;c<e;c++)w(a.children[c],
...
!1);this.controls.addEventListener("change",this.render);this.render()};makeItZoom.prototype.render=function(){this.renderer.render(this.scene,this.camera);var a=new CustomEvent("mz_render");a.mz_center_x=this.controls.object.position.x;a.mz_scale=this.controls.currentZoomScale;this.container.dispatchEvent(a);a.mz_center_y=this.controls.object.position.y};makeItZoom.prototype._addZoomable=function(a,c){var e=new THREE.CSS2DObject(a);e.position.x=c.left;e.position.y=c.top;e.position.z=0;this.scene.add(e)};
